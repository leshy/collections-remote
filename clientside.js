...
(function() {
  ...
  if (!global.window) {
    ...
    post = function(url, data, callback) {
      var gotData, gotErr, gotProgress, xhr;
      xhr = new XMLHttpRequest;
      xhr.open("POST", url);
      xhr.setRequestHeader("Content-Type", "application/json;charset=UTF-8");
      gotData = function() {
        return callback(void 0, JSON.parse(xhr.responseText));
      };
      gotErr = function() {
        return callback(true);
      };
      gotProgress = function(oEvent) {
        var percentComplete;
        if (oEvent.lengthComputable) {
          return percentComplete = oEvent.loaded / oEvent.total;
        }
      };
  ...
  RemoteCollectionHttp = exports.RemoteCollectionHttp = Backbone.Model.extend4000({
    ...
    find: function(pattern, limits, callback, callbackDone) {
      ...
      post(this.getpath('find'), {
        ...
        return helpers.cbc(callbackDone, err, true);
      });
      return void 0;
    },
