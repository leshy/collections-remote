...
(function() {
  var Backbone, CollectionExposerHttp, helpers;
  ...
  CollectionExposerHttp = exports.CollectionExposerHttp = Backbone.Model.extend4000({
    initialize: function() {
      ...
      app.post(helpers.makePath(path, name, 'find'), function(req, res) {
        return c.findModels(req.body.pattern, req.body.limits, function(model) {
          if (model) {
            return res.write(JSON.stringify(model.attributes));
          } else {
            return res.end();
          }
        });
